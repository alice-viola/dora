...
module.exports = class Workload extends R.Resource {
    ...
    pause () {
        if (this._p.currentStatus !== R.GE.WORKLOAD.RUNNING) {
            return
        }
        ...
        if (this._p.spec.selectors !== undefined) {
            ...
            // Should not be possible, but for the sake of security..
            this._p.spec.selectors = {node: {name: this._p.scheduler.node}}
        }
    ...
    replaceImage () {
        if (this._p.scheduler !== undefined && this._p.scheduler.container !== undefined) {
    ...
    currentRunningTime () {
        ...
        return millisToMinutes(new Date() - new Date(this._p.status[this._p.status.length -1].data)) 
    }
