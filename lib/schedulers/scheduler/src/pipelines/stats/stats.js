...
pipe.step('metric', (pipe, job) => {
	...
	pipe.data.resources.forEach((node) => {
		...
		*/
		...
		// Booked GPU
		// stats.usage.assignedGpu = []
		// pipe.data.workloads.forEach ((wk) => {
		// 	if (wk.currentStatus == GE.WORKLOAD.RUNNING || wk.currentStatus == GE.WORKLOAD.REQUESTED_LAUNCH || wk.currentStatus == GE.WORKLOAD.ASSIGNED) {
		// 		if (wk.scheduler !== undefined && wk.scheduler.gpu !== undefined && wk.scheduler.node == node.name ) {
		// 			wk.scheduler.gpu.forEach((gpu) => {
		// 				stats.usage.assignedGpu.push({product_name: gpu.product_name, uuid: gpu.uuid, minot_number: gpu.minor_number})
		// 			})
		// 		}
		// 	}
		// })
