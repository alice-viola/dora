...
let FN = {
	...
	VolumeName: [{
    	...
    	type: 'input',
    	name: 'volumeName',
    	message: "Insert your volume name",
    	default: 'no volume'
	}],
	...
	VolumePath: [{
    	...
    	type: 'input',
    	name: 'volumeTarget',
    	message: "Insert where you want to mount the volume",
    	default: '/home'
	}],
	...
	CmdPath: [{
    	...
    	type: 'input',
    	name: 'command',
    	message: "Insert the command to start your container",
    	default: 'As Dockerfile CMD'
	}],
...
function generatePipe () {
	return {
		Init: 				{fn: FN.Init, next: (res) => { return pipe.ImageRegistry  }},
		...
		VolumeName: 		{fn: FN.VolumeName, next: (res) => { return res == 'no volume' ? pipe.CmdPath : pipe.VolumePath }},
		VolumePath: 		{fn: FN.VolumePath, next: (res) => { return pipe.CmdPath }},
		CmdPath: 			{fn: FN.CmdPath, next: (res) => { return pipe.WorkloadName }},
		WorkloadName: 		{fn: FN.WorkloadName, next: (res) => { return undefined }},
	}
}
let pipe = generatePipe()

function next (key, res) {
	if (key == undefined || pipe[key] == undefined) {
		return undefined
	}
	return pipe[key].next(res).fn
}
...
function execResults (responses) {
	...
	let computeConfig = (_responses) => {
		if (_responses.command !== 'As Dockerfile CMD') {
			return {cmd: _responses.command}
		} else {
			return null
		}
	}
	...
	return {
		...
		metadata: {
			name: generatedName,
			group: 'prom-lab' // TODO not hardcoded
		},
		spec: {
			selectors: {
				gpu: {
					product_name: 'GeForce GTX 1080 Ti',
					count: responses.numberOfGpu
				}
			},
			image: computeImage(responses),
			config: computeConfig(responses),
			volumes: [
				{name: responses.volumeName, target: responses.volumeTarget, storage: 'pwmzfs01'}
			]
		}
	}
}
